 ...
 */
async function generateNotifications(companyId) {
    const now = new Date();
    ...
    for (const lc of expiringLCs) {
        ...
        if (!existing) {
            ...
            const convRate = lc.conversionRate || 1;
            await database_1.default.notification.create({
                data: {
    ...
    const pendingCount = await database_1.default.journalEntry.count({ where: { companyId, status: 'PENDING_VERIFICATION' } });
    if (pendingCount > 0) {
        const existing = await database_1.default.notification.findFirst({
...
class DashboardController {
    async getStats(request, reply) {
        ...
        try {
            ...
            const payLines = await database_1.default.journalEntryLine.findMany({
                where: {
                    journalEntry: { companyId, status: 'APPROVED' },
                    account: {
            ...
            const loanLines = await database_1.default.journalEntryLine.findMany({
                where: {
                    journalEntry: { companyId, status: 'APPROVED' },
                    account: {
                        accountType: { name: 'LIABILITY' },
            ...
            const revenueAccounts = await database_1.default.account.findMany({
            ...
            const customerReceivables = await database_1.default.customer.findMany({
            ...
            // Payables Breakdown
            const vendorPayables = await database_1.default.vendor.findMany({
                where: { companyId, isActive: true },
                include: { journalLines: { where: { journalEntry: { status: 'APPROVED' } } } }
            });
            ...
            const loanRecords = await database_1.default.loan.findMany({
            ...
            if (loanRecords.length > 0) {
                ...
                const loanAccounts = await database_1.default.account.findMany({
                    where: {
                ...
                loanBreakdown = loanAccounts.map((acc) => ({
                    ...
                    outstandingBalance: acc.journalLines.reduce((sum, l) => sum + (Number(l.creditBase) - Number(l.debitBase)), 0),
                    monthlyInstallment: 0,
                    endDate: null
                })).filter((a) => Math.abs(a.outstandingBalance) > 0.01);
            }
            // --- New: Buyer-wise Distribution for Pie Chart ---
            const buyers = await database_1.default.customer.findMany({
                where: { companyId, isActive: true },
                include: {
                    journalLines: {
                        where: {
            ...
            const cashFlowData = await (async () => {
                const getCashFlowForPeriod = async (from, to) => {
                    const lines = await database_1.default.journalEntryLine.findMany({
                        where: {
                            journalEntry: { companyId, status: 'APPROVED', date: { gte: from, lte: to } },
                    ...
                    for (const line of lines) {
                        ...
                        if (cfType === 'OPERATING') {
                            if (amount > 0)
                                operating.inflows += amount;
                            else
                                operating.outflows += Math.abs(amount);
                        }
                        else if (cfType === 'INVESTING') {
                            if (amount > 0)
                                investing.inflows += amount;
                            else
                                investing.outflows += Math.abs(amount);
                        }
                        else if (cfType === 'FINANCING') {
                            if (amount > 0)
                                financing.inflows += amount;
                            else
                                financing.outflows += Math.abs(amount);
                        }
                    }
            ...
            let cumulativeCash = cashFlowData.openingCash - (await (async () => {
                ...
                for (let i = 1; i <= 5; i++) {
                    ...
                    const lines = await database_1.default.journalEntryLine.findMany({
                        where: { journalEntry: { companyId, status: 'APPROVED', date: { gte: start, lte: end } }, account: { cashFlowType: { not: null } } }
                    });
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (const l of lines) {
                    ...
                    if (l.account.cashFlowType === 'OPERATING')
                        opNet += val;
                    else if (l.account.cashFlowType === 'INVESTING')
                        invNet += val;
                    else if (l.account.cashFlowType === 'FINANCING')
                        finNet += val;
                }
            ...
            const todayLines = await database_1.default.journalEntryLine.findMany({
                where: {
                    account: {
                        companyId,
                        accountType: { name: 'ASSET' },
            ...
            if (isOwner || isAdmin) {
                ...
                dashboardData.kpis = {
                    dailyCollection,
                    todayPayments: 0,
                    pendingVouchers: await database_1.default.journalEntry.count({ where: { companyId, status: 'PENDING_VERIFICATION' } }),
                ...
                dashboardData.kpis = {
                    pendingVouchers: await database_1.default.journalEntry.count({ where: { companyId, status: 'DRAFT' } }),
                    unpostedEntries: await database_1.default.journalEntry.count({ where: { companyId, status: 'PENDING_VERIFICATION' } }),
                    unreconciledBank: 0
                };
                dashboardData.actions = [
