...
import { Building2, FileText, CreditCard, Clock, AlertCircle, Loader2, LogOut } from 'lucide-react';
...
export default function CustomerPortalPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['customer-portal', companyId, token],
    queryFn: () => api.get(`/portal/${companyId}/customer/${token}`).then(r => r.data.data),
  ...
  const { customer, invoices, payments, aging, summary } = data;
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-slate-900">{customer.companyName || 'Customer Portal'}</h1>
          ...
          <div className="p-6">
            {activeTab === 'summary' && (
              <div className="text-center py-8">
                <p className="text-slate-500">Select a tab above to view details</p>
              </div>
            )}
