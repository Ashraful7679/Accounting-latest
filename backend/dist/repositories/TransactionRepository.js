...
const sequence_service_1 = require("../modules/company/sequence.service");
...
class TransactionRepository {
    ...
    // --- Purchase Bill Hooks ---
    static async generateBillJournal(tx, bill, companyId, userId) {
        ...
        if (!apAccount || !expAccount)
            throw new Error('Accounts Payable or Expense account not found');
    ...
    // --- Payment Hooks ---
    static async generatePaymentJournal(tx, payment, companyId, userId, type) {
        ...
        if (!arApAccount)
            throw new Error(`${arApCategory} account not found`);
        if (!payment.accountId)
            throw new Error('Settlement account (Bank/Cash) not found on payment');
    ...
    // --- Transfer Hooks ---
    static async generateTransferJournal(tx, transfer, companyId, userId, toAccountId) {
    ...
    static async getAccountTypeId(typeName) {
        const type = await database_1.default.accountType.findUnique({ where: { name: typeName.toUpperCase() } });
        if (!type)
            throw new Error(`Account type ${typeName} not found`);
        return type.id;
    }
    /**
     ...
     */
    static async ensureEntityAccount(tx, companyId, entityId, entityName, entityCode, category, openingBalance = 0) {
        ...
        while (true) {
            ...
            counter++;
            if (++attempts > 200)
                throw new Error(`Cannot generate unique account code for category "${category}" after 200 attempts`);
        }
     ...
     */
    static async generateSalaryJournal(tx, { companyId, employeeId, amount, date, description, userId }) {
        ...
        if (!salaryExpenseAccount) {
            ...
            salaryExpenseAccount = await tx.account.create({
                data: {
                    code: await sequence_service_1.SequenceService.generateDocumentNumber(companyId, 'product', tx), // PRD used as fallback for generic accounts
