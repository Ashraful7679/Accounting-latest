...
export default function PurchaseInvoicesPage() {
  ...
  const piSearchParams = useSearchParams();
  const [mounted, setMounted] = useState(false);
  const [activeTab, setActiveTab] = useState<'local' | 'foreign'>('local');
  
  useEffect(() => {
    const type = piSearchParams?.get('type');
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-end bg-white p-6 rounded-sm border border-gray-200 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Manage Local & Foreign Supplier Bills</p>
        </div>
        <Link 
          href={`/company/${companyId}/purchase/invoices/create?type=${activeTab}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-sm p-2 flex items-center justify-center shadow-sm">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            Total Invoices: <span className="text-gray-900">{filteredInvoices.length}</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 font-mono">LOADING INVOICES...</td></tr>
            ) : filteredInvoices.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 font-mono uppercase tracking-widest">No matching records found</td></tr>
            ) : (
                ...
                <tr key={inv.id} className="hover:bg-gray-50/50 group transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-gray-700 uppercase tracking-tight">{inv.vendor?.name || '-'}</span>
                      {inv.lcId && <span className="text-[8px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-sm font-black uppercase">LC Linked</span>}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {/* Only show Payment for Local (BDT) or Non-LC invoices directly here */}
