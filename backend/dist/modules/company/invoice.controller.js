...
class InvoiceController extends base_controller_1.BaseCompanyController {
    // ============ INVOICES ============
    ...
    async createInvoice(request, reply) {
        ...
        try {
            ...
            if (!data.lines || !Array.isArray(data.lines)) {
                throw new errorHandler_1.ValidationError('Invoice lines are required');
            }
            const subtotal = data.lines.reduce((sum, line) => sum + (line.quantity * line.unitPrice), 0);
            const taxAmount = data.lines.reduce((sum, line) => sum + (line.quantity * line.unitPrice * (line.taxRate || 0) / 100), 0);
    ...
    async submitInvoice(request, reply) {
        const { invoiceId } = request.params;
        const { id: companyId } = request.params;
        const userId = request.user.id;
        const role = await this.getUserRole(userId, companyId);
        if (role !== 'Accountant' && role !== 'Owner' && role !== 'Admin') {
            throw new errorHandler_1.ForbiddenError('Insufficient permissions to submit invoices');
        }
        const invoice = await database_1.default.invoice.findUnique({ where: { id: invoiceId } });
        if (!invoice)
            throw new errorHandler_1.NotFoundError('Invoice not found');
        if (invoice.status !== 'DRAFT' && invoice.status !== 'REJECTED') {
            throw new errorHandler_1.ValidationError('Only DRAFT or REJECTED invoices can be submitted');
        }
        const updated = await database_1.default.invoice.update({
            where: { id: invoiceId },
